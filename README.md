...
For large sidecar-style deployments where hundreds of TiProxy instances would otherwise poll PD directly, TiProxy can instead fetch the TiDB topology from the `tidb-discovery` service (a PD-side component started with `pd-server services tidb-discovery`) over HTTP, keeping the load on PD constant. See the [design doc](docs/design/discovery-hub.md) and the [operations guide](docs/design/discovery-hub-ops.md).
